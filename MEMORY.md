...
- **4RFV database quarantined:** removed from git tracking, moved to gitignored `reference/` with recorded non-use decision (sui generis DB right + scraped personal data). ⚠ Still in pushed GitHub history — **purge decision pending with principal** (git filter-repo + force push; lab's clone pins old hashes).
- March working tree committed as 4 logical commits and pushed (e927b5f → 9078b00).
