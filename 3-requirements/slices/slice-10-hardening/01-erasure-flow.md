...
**Sections:** §1 GDPR Erasure Flow Wiring, §2 processErasure Implementation
...
async function processErasureStep(context: ErasureContext): Promise<void> {
  ...
  // Sub-steps 2–3 (R2 cleanup + quality recalculation): **See §2.7 for the authoritative
  // `processErasureStep` implementation** (revised to capture `claimIdsForR2Cleanup`
  // pre-transaction). [S10-ST-3]
}
```
...
resolveDisputes(tx, accountId):
  ...
  disputedListingIds = disputedOwned.map(l => l.id)
  snapshots = tx.select(pre_claim_snapshots)
    .where(listingId IN disputedListingIds)
  snapshotMap = new Map(snapshots.map(s => [s.listingId, s]))
...
processListings(tx, accountId):
  ...
  freelancerListings = ownedListings.filter(l => l.entityType === "freelancer")
  companyListings = ownedListings.filter(l => l.entityType !== "freelancer")

  // Batch delete freelancer listings (FK cascades handle child rows)
  if (freelancerListings.length > 0):
    freelancerIds = freelancerListings.map(l => l.id)
    tx.delete(listings).where(id IN freelancerIds)
    deleted.push(...freelancerIds)

  // Company listings require per-listing anonymisation (child table deletes differ)
  for listing in companyListings:
...
async function processErasureR2Cleanup(
  ...
  //       claimIds captured pre-transaction by processErasureStep (§2.2)
