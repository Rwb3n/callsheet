...
**Sections:** §7 Autonomy Graduation (S9-1, S9-2 resolution), §8 Algorithm Versioning & Controlled Rollout (S9-3 resolution)
...
| Enrichment ROI | >0.5 (majority improve) | True positive rate: proportion of adjustments that led to quality score improvement within 90 days |
...
async function evaluateEnrichmentCadenceGraduation(): Promise<GraduationDecision> {
  ...
  // ROI: proportion of enrichment cadence adjustments that led to quality improvement.
  // V1 proxy: count adjustments where listing quality score increased within 90 days
  // (same quality_scores query used for false positive detection above — reuse results).
  // A "true positive" is an adjustment NOT counted as a false positive.
  const truePositives = adjustments.length - falsePositives
  const enrichmentROI = truePositives / adjustments.length  // 0.0–1.0, threshold >1.0 unreachable
  // Note: threshold adjusted to >0.5 (majority of adjustments produce quality improvement).
  // Full ROI measurement (linking enrichment cost to quality delta) deferred to post-launch.

  const thresholds = { falsePositiveRate: 0.02, enrichmentROI: 0.5 }
