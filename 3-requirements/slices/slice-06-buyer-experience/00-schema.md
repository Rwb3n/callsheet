...
// Retention: 12 months, search_history_cleanup deferred action (SI §2.1)
```
...
S6 reads `status` to render shortlist items with correct display state. PP consumers [XP-15] write lifecycle transitions (`active` -> `archived`/`suspended` and back). S6 writes only on buyer actions: `shortlist_added` (insert row with `status = "active"`) and shortlist item removal (`status` -> `"removed"`).
