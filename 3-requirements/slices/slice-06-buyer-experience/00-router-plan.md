...
type ShortlistItemWithListing = {
  ...
  listing: {                       // joined from listings table — batch query (Decision D2)
    slug: string
    name: string
    headline?: string
    entityType: EntityType
...
|------|----------|-----------|
| `/search` | SSR | Query-dependent, SEO-relevant (search results pages indexable), results must be fresh. Target: <500ms TTFB p95 [SI §12.1]. |
