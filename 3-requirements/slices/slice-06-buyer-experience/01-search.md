...
| AC-13 | `search_performed` event emitted with exact `SearchPerformedEvent` payload: `{ type, query, filters, resultCount, sessionId?, timestamp }`; `sessionId` is `ctx.session.accountId` for authenticated users, `null` for anonymous [S6-ST-4] | Integration |
