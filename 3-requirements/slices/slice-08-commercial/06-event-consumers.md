...
async function handleSubscriptionTierChanged(
  ...
  if (direction === "upgrade" && previousTier !== "free") {
    // Check if first_upgrade already emitted — milestone is once-per-listing
    const existingLog = await db.query.churnAnalysisLog.findFirst({
      where: and(
...
async function handleQualityScoreChanged(
  ...
  // Only interested in quality below 60 (low_quality_paid < 40, quality_declining 40-59)
  if (newComposite >= 60) return
...
**P1 compliance:** `triggerLowQualityIntervention` (§7) handles `churn_risk_detected` emission for `low_quality_paid` — P1 compliance verified in §7's content section. Direct `churn_risk_detected` emission for `quality_declining` [S8-ST-5] matches CR §1.2 payload: `listingId`, `accountId`, `riskFactors`, `timestamp`. The `listings.subscriptionStartDate` read is a documented local-state join per D1, not a cross-domain DB read in handler [Source: CR §2 notes].
