...
type ConversionTriggerState = {
  ...
  analyticsTeaseFired: number              // firing count
  socialProofFired: number                 // firing count
  engagementSummaryFired: number           // firing count
...
| `platform-and-product.md` (v6 interface) | §3.1 `getListingAnalytics` — consumed by win-back evaluation (§3) and EnquiryResponseInsights (CR §5 shared type). Not consumed by RevenuePerception computation (§5). |
