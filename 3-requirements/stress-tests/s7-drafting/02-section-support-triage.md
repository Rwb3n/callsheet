...
**Skeleton section:** §2
**Inputs:** `01-schema.md` (§2.1 `support_tickets`, §2.3 `churn_risk_registry`), `01-router-plan.md` (§3.2), `01-decisions.md` (D1, D4), `shared-infrastructure.md` (v6) §4.1, §5.2, §8, §9, `operations.md` (v6) §4, `operations.md` (v3 interface) §3.1
...
classifyTicket(request: { subject: string; body: string; accountId?: UUID; listingId?: UUID }): TicketCategory
  ...
  if matches(subjectAndBody, ["invoice", "billing", "payment", "refund", "charge", "subscription"]):
...
| # | Criterion |
|---|-----------|
| AC-2.1 | `classifyTicket` assigns one of 7 categories based on keyword matching against subject + body |
| AC-2.2 | Base priority is deterministic per category: `data_request` and `claim_dispute` = high; `billing_support` and `account_access` = normal; `feature_gating_confusion` and `profile_support` = low; `other` = normal |
...
| AC-2.8 | KB deflection returns a suggested article URL for `account_access`, `billing_support`, `feature_gating_confusion`, and `profile_support` categories |
