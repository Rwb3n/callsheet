...
**Phase:** 2 (Content)
**Agent:** Cross-Role + Gating
**Written:** 2026-02-14
**Inputs:** `s6-pre-draft-checklist.md` §9, `01-router-plan.md` §2.6, `01-schema.md` §1.1, `01-decisions.md`, `platform-and-product.md` §7.3, `commercial-and-revenue.md` §4.1–§4.3, `slice-04-subscriptions.md` §4.1–§4.2, `slice-05-provider-experience.md` §11
...
function shouldShowEngagementStats(listing: ListingProfileData): boolean {
  ...
  return access.basicAnalytics === true && listing.subscriptionTier !== "free"
  // basicAnalytics is always true, but engagement stats on the PUBLIC profile
  // are a Standard+ feature. The provider always sees their own stats in S5 dashboard.
  // This gate controls buyer-visible display only.
}
```

**Implementation note:** `basicAnalytics: true` in `FeatureAccess` means the *provider* always has access to their own basic analytics (total views, enquiries). The buyer-facing display of engagement stats on the *public profile page* is a separate concern — it controls whether social proof is visible to other users. This distinction is enforced in profile page rendering, not in `computeFeatureAccess`.
