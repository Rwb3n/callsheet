...
**Phase:** 2 (Content)
**Agent:** Shortlist + Dashboard
**Written:** 2026-02-14
**Inputs:** `01-schema.md` (§2.1 no-amendment decision, §4.2 shortlist_items snapshot), `01-router-plan.md` (§2.2 shortlist router, §2.3 enquiry.listSent), `01-decisions.md` (D2 no displayStatus, D3 enquiry_responded handled by S5), `s6-pre-draft-checklist.md` (§4, §5, §8), `platform-and-product.md` (v5 §6.1, §7.2), `slice-01-data-model.md` (v2 §2.2 shortlists/shortlist_items), `slice-05-provider-experience.md` (v2 §5.3 enquiry status lifecycle)
...
The `shortlist_items.status` column tracks only the item's own lifecycle within the shortlist: `active` (in shortlist) or `removed` (buyer removed). The `archived` and `suspended` values in `shortlistItemStatusEnum` (S1-ST-7) exist in the enum but are unused by S6 — listing state is read from `listings.lifecycleStatus` via join, not written to `shortlist_items.status` by event consumers.
  ...
  return {
    items: items.map(row => ({
      ...
      listing: {
        ...
        lifecycleStatus: row.lifecycle_status,       // read from listings, not shortlist_items
      },
    })),
...
type ShortlistItemWithListing = {
  ...
  listing: {
    ...
    baseRegion?: string
    headshotUrl?: string
    lifecycleStatus: LifecycleStatus                 // from listings table via join — NOT a shortlist_items column
  }
}
```

**UI rendering rules for listing state:**

| `listings.lifecycleStatus` | Display Treatment |
|---------------------------|-------------------|
...
| `inactive` / `dissolved` / `merged` | Dimmed card, "No longer active" badge |

### 4.5 Listing Removal by Event

Two event-driven paths affect shortlist items. Neither requires S6 to register new consumers or update `shortlist_items` rows.

**`erasure_completed`:** When a listing is permanently erased, `shortlist_items` rows cascade-delete via the FK constraint `listings.id → onDelete: "cascade"` (S1 §2.2). The buyer's next `shortlist.getItems` call returns fewer results — no stale references remain.

**`listing_archived` / `listing_suspended`:** These events change `listings.lifecycleStatus` on the listing itself. S6's read-time join (`shortlist.getItems` query in §4.4) automatically reflects the new state — the query reads `listings.lifecycle_status` directly. No consumer writes to `shortlist_items` are needed. When `listing_reactivated` fires, the listing's `lifecycleStatus` returns to `active`, and the join reflects this on the next read.

```mermaid
flowchart LR
    A[listing_archived fires] --> B[listings.lifecycleStatus → archived]
    B --> C[shortlist.getItems JOIN reads lifecycle_status]
