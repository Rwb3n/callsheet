...
**Content Agent:** 4
**Scope:** All recurring ceremony handlers — D&L (taxonomy review, data health, verification calibration, provider outreach), CR (conversion funnel, multi-listing pricing), Ops (operational health, contractor performance, principal briefing). Credit confirmation outreach. Enrichment confirmation request. Ceremony framework (shared pattern, idempotency, outcome evaluation).
**Authoritative for:** Ceremony handler pseudocode, `ceremony_outcome_evaluation` decision architecture, `taxonomy_promotion_evaluation` decision architecture. §6 (Event Consumers) is authoritative for consumer handler code that invokes these ceremonies.
**Sources:** `2-concept-design/data-and-listings.md` §5 (ceremonies), `2-concept-design/operations.md` §9 (ceremonies + PrincipalBriefing type), `01-schema.md` (ceremony_runs, learning_hypotheses, principal_briefings tables), `01-decisions.md` (D3, D4, D6), `01-router-plan.md` §3 (deferred action inventory)
...
handleMultiListingPricingEvaluation():
  ...
  multiListingPaidAccounts = count(
    ...
    AND subscription_tier IN ('standard', 'professional', 'premium')
    GROUP BY account_id
    HAVING count(*) > 1
  )
...
type OperationalHealthReport = {
  ...
  healthTrends: {
    ...
    decaySignals30d: number
    qualityChanges30d: number
  }
}
```
...
handlePrincipalBriefingGeneration():
  ...
  // Send email — principal_briefing template, category: internal
...
| AC-S9-4-11 | `principal_briefing_generation` sends `principal_briefing` email template to the principal after storing the briefing. `sentAt` column updated on successful send. | integration |
