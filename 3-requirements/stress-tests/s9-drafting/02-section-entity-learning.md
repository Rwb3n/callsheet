<!-- Part of slice-09-entity-intelligence v1 — Content Agent 5 output -->

# SS5 Entity Learning & Commercial Intelligence

S9 SS5 implements entity learning feedback loops and advanced commercial intelligence — the mechanisms by which the entity observes its own decision history and calibrates future behaviour. Seven subsystems: L1-L7 hypothesis measurement, proactive churn detection (producing the 2/5 `ChurnRiskFactor` values S8 deferred), sponsored placement learning, conversion-denominated friction ratios, extended revenue health metrics, operational health review, and contractor performance review.

All subsystems are deferred-action-driven. No user-facing routes — outputs surface via `admin.intelligence.learningHypotheses` and `admin.intelligence.revenueHealth` (router plan SS2.1). Decision logging via SI SS9.2 `logDecision()` is mandatory for every subsystem that evaluates signals. [Source: `01-decisions.md` D8]

**Decision types registered:** `proactive_churn_detection`, `conversion_threshold_adjustment`. [Source: `01-decisions.md` D8, SI SS9.2]
...
onConversionMilestone(event: ConversionMilestoneEvent):
  ...
  triggerGate = event.metadata?.triggerGate ?? "unknown"
