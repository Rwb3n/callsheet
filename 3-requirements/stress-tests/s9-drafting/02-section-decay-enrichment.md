...
**Concept design source:** D&L CD §3 (decay detection pipeline, enrichment cadence, liveness checks)
**Interface sources:** D&L §1.7 (`DecaySignalDetectedEvent`), Ops §3.1 (`hasActiveTicket`), SI §2.1 (`DeferredActionParamsMap`), SI §9.2 (`logDecision`)
**Schema source:** `01-schema.md` — `enrichment_schedules`, `decay_signals` tables, `decay_signal_type`, `decay_signal_severity`, `enrichment_check_type` pgEnums
...
evaluateDecayResponse(listingId, signal, existingSignals, activeSupportTicket):
  ...
  if signal.severity == "critical":
    ...
    decision = {
      ...
      supportTicketCreated: false,
      searchRankingDownweight: false,
      reason: "Medium severity — quality score impact, no notification"
    }
...
**Unsubscribable:** No (operational — listing at risk of archival)
**Owner:** D&L
