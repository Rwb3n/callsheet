# S9 Phase 1B: Schema Definitions

**Status:** Phase 1 foundation output
**Slice:** S9 — Entity Intelligence
**Generated:** 2026-02-15
**Purpose:** Complete Drizzle ORM schema definitions for all S9 additions (6 new tables, 2 column amendments, 4 new pgEnums).
...
- **Total tables:** 44 (38 from S8 + 6 new)
...
40. *(Note: `pending_cancellations` is counted under S4 — S7 reuses it)*

**S8: Commercial (3 tables)**

41. `commercial_state`
42. `churn_analysis_log`
43. `sponsored_impressions`

**S9: Entity Intelligence (6 tables)**

44. `enrichment_schedules`
45. `decay_signals`
46. `perception_aggregates`
47. `ceremony_runs`
48. `learning_hypotheses`
49. `principal_briefings`

**Corrected total: 44 tables** (S0 5 + S1 14+8+2 + S4 3 + S6 1 + S7 4 new + S8 3 + S9 6 = 46? Recount: S7 adds 7 tables: support_tickets, task_specs, churn_risk_registry, billing_holds, compliance_register, billing_reconciliation_status — but pending_cancellations is S4. So S7 = 6 new. 5+24+3+1+6+3+6 = 48? Error in cumulative schema count.)

**Reconciliation:** Cumulative schema from S8 lists 42 tables (line 873: "Total: 38 tables" then says "S8 adds 3 = **38**" — this is an error). Recount from cumulative snapshot:

- S0: 5 (deferred_actions, orchestrated_flows, notifications, decision_logs, event_consumer_errors)
- S1 D&L: 14 (listings through zero_result_queries per §1.2–§1.15)
- S1 Account/Buyer: 5 (account_profiles, shortlists, shortlist_items, saved_searches, enquiry_records)
- S1 Search: 2 (search_synonyms, controlled_vocabulary)
- S4: 3 (pending_cancellations, processed_paddle_events, grace_periods)
- S6: 1 (search_history)
- S7: 7 (support_tickets, task_specs, churn_risk_registry, pending_cancellations [S4 overlap — not counted], billing_holds, compliance_register, billing_reconciliation_status)

Wait, S7 §2 in cumulative schema lists 7 tables but pending_cancellations is under S4. So S7 net new = 6 (support_tickets, task_specs, churn_risk_registry, billing_holds, compliance_register, billing_reconciliation_status).

- S8: 3 (commercial_state, churn_analysis_log, sponsored_impressions)

Total: 5 + 14 + 5 + 2 + 3 + 1 + 6 + 3 = **39 tables** (not 38).

**S9 adds 6 new tables → total after S9: 45 tables** (not 44).

**Corrected cumulative count: 45 tables.**
...
- S1: 12 (`entity_type`, `claim_status`, `verification_tier`, `lifecycle_status`, `subscription_tier`, `availability_status`, `travel_willingness`, `budget_tier`, `lead_time`, `credit_format`, `credit_sourcing`, `verification_method`, `shortlist_item_status`, `media_type`, `vocabulary_category`) — 15 total in §1.1, cumulative snapshot shows 12 in comments. Recount from cumulative: entity_type, claim_status, verification_tier, lifecycle_status, subscription_tier, availability_status, travel_willingness, budget_tier, lead_time, credit_format, credit_sourcing, verification_method, shortlist_item_status, media_type, vocabulary_category = **15**.
...
Total: 4 + 15 + 1 + 1 + 7 + 0 + 4 = **32 pgEnums** (cumulative snapshot says 32 from S8, which is correct if S1 = 12 not 15. Error in cumulative snapshot or count above.)
...
## 9. Authoritative for Phase 2 Content Agents
